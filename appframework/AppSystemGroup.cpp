...
{
	HMODULE tier0_new = nullptr;

	for (int i = 0; i < m_Systems.Count(); ++i )
	{
		// F1ssi0N: HACK FOR FILESYSTEM
		// filesystem wants the new ICommandLine interface whilst we use the old one
		// it would be better if we used the new one - this wouldnt happen
		void **oldVtable = nullptr;
		if (m_Systems[i]->QueryInterface(FILESYSTEM_INTERFACE_VERSION) != nullptr)
		{
			void ***pVmt = (void ***)CommandLine();
			// store the old vtable address
			oldVtable = *pVmt;

			if (tier0_new == nullptr)
				tier0_new = LoadLibraryA("tier0_filesystem.dll");

			*pVmt = *(void ***)GetProcAddress(tier0_new, "CommandLine_Tier0")();
		}
