...
{
	CDmrElementArray<CDmElement> array( pOther );
...
bool CImportVMF::SerializeEntities( CUtlBuffer &buf, CDmAttribute *pEntities )
{
	...
	// to minimize diffs
	CDmrElementArray<CDmElement> array( pEntities );
...
bool CImportVMF::UnserializeEntityKey( CDmAttribute *pEntities, KeyValues *pKeyValues )
{
	...
	CDmrElementArray<CDmElement> otherKeys( pEntity->AddAttribute( "other", AT_ELEMENT_ARRAY ) );
